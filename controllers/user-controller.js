...
const { generateHashPassword, generateAuthToken, checkHashPassword} = require('../services/token-service');
...
const loginController = async (req, res, next) => {
 ...
 if(user) {
  ...
  if(isMatch){
    ...
    return res.status(200).json({status:true, message:"Authentication Successful", user:{id:user._id, token}});
  } else {
